...
Usage:
    ...
    torchrun --nproc_per_node=1 train.py \
        --dataset data/train_dataset.npz --ckpt-dir logs/my_task

    # Multi-GPU (2 GPUs)
    torchrun --nproc_per_node=2 train.py \
        --dataset data/train_dataset.npz --ckpt-dir logs/my_task --batch-size 48

    # Without torchrun (single GPU, no DDP)
    python train.py --dataset data/train_dataset.npz --ckpt-dir logs/my_task
"""
...
# Optional: cosine schedule with warmup from diffusers
try:
...
def train(args):
    ...
    if use_ddp and WORLD_SIZE > 1:
        ...
        # torchrun with nproc_per_node=1: still init for compatibility
        dist.init_process_group(
            backend="nccl", init_method="env://",
            world_size=WORLD_SIZE, rank=RANK,
        )
    ...
    if len(dataset) == 0:
        raise RuntimeError("Dataset is empty — check --data-dir")
